...
class PrincipalApplicationController extends Controller
{
    ...
    private function ensureTenantScope(): void
    {
        $principal = $this->getPrincipal();
        $tenant = app(TenantManager::class);

        if (!$tenant->isResolved() || !$tenant->school() || $tenant->school()->id !== $principal->school_id) {
            abort(500, 'Tenant database not resolved for this school admin.');
        }

        // Ensure principal exists in tenant DB for FK integrity (reviewed_by)
        app(TenantUserSyncService::class)->sync($principal);
    }
    ...
    public function show($application)
    {
        ...
        $school = School::findOrFail($principal->school_id);
    ...
    public function print($application)
    {
        $this->ensureTenantScope();
        $principal = $this->getPrincipal();
    ...
    public function approve(Request $request, $application)
    {
        ...
        $validated = $request->validate([
            'school_class_id' => [
                'required',
                \Illuminate\Validation\Rule::exists('tenant.school_classes', 'id')
        ...
        DB::transaction(function () use ($application, $validated, $principal, $class, &$generatedEmail, &$generatedPassword) {
            ...
            // 3b. Mirror user into tenant DB for FK integrity
            app(TenantUserSyncService::class)->sync($user);
        ...
        $application->refresh();
        app(DecisionDocumentService::class)->generateForApplication($application, 'accepted', $principal);
    ...
    public function reject(Request $request, $application)
    {
        ...
        $application->refresh();
        app(DecisionDocumentService::class)->generateForApplication($application, 'rejected', $principal);

        return redirect()->back()->with('success', 'Application has been rejected.');
    }

    public function document($application, $document): BinaryFileResponse
    {
        $this->ensureTenantScope();
        $principal = $this->getPrincipal();
        ...
        $document = StudentApplicationDocument::findOrFail($document);
