...
export default function Show({ auth, school, application, classes, credentials, documents = [] }) {
    ...
    const guardian = typeof data.guardian === 'object' && data.guardian !== null ? data.guardian : {};
    const previousSchool = typeof data.previous_school === 'object' && data.previous_school !== null ? data.previous_school : {};
    const guardianName = data.guardian_name ?? guardian.name ?? '—';
    const guardianOccupation = data.guardian_occupation ?? guardian.occupation ?? '—';
    const guardianPhone = data.guardian_phone ?? guardian.phone ?? '—';
    const guardianEmail = data.guardian_email ?? guardian.email ?? '';
    const guardianAddress = data.guardian_address ?? guardian.address ?? '—';
    const previousSchoolName = data.previous_school_name
        ?? (typeof data.previous_school === 'string' ? data.previous_school : null)
        ?? previousSchool.name
        ?? '';
    const previousSchoolLastClass = data.previous_school_last_class ?? previousSchool.last_class ?? '';
    const previousSchoolYear = data.previous_school_year_completed ?? previousSchool.year_completed ?? '';
    const dateOfBirth = data.date_of_birth || '';
    const dobDate = dateOfBirth ? new Date(dateOfBirth) : null;
    const age = dobDate && !Number.isNaN(dobDate.getTime())
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
                                    <dl className="sm:divide-y sm:divide-gray-200">
                                        ...
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{data.place_of_birth || '—'}</dd>
                                        </div>
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                        ...
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{data.district || '—'}</dd>
                                        </div>
                                    </dl>
                                </div>
                            </div>
                                ...
                                <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
                                    <dl className="sm:divide-y sm:divide-gray-200">
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{guardianName}</dd>
                                        </div>
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                            <dt className="text-sm font-medium text-gray-500">Occupation</dt>
                                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{guardianOccupation}</dd>
                                        </div>
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                            <dt className="text-sm font-medium text-gray-500">Phone Number</dt>
                                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{guardianPhone}</dd>
                                        </div>
                                        {guardianEmail && (
                                            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                <dt className="text-sm font-medium text-gray-500">Email Address</dt>
                                                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{guardianEmail}</dd>
                                            </div>
                                        )}
                                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                            <dt className="text-sm font-medium text-gray-500">Address</dt>
                                            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{guardianAddress}</dd>
                                        </div>
                                    </dl>
                                </div>
                            </div>
                            ...
                            {/* Academic History */}
                            {(previousSchoolName || previousSchoolLastClass || previousSchoolYear || application.class_category !== 'primary') && (
                                <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                                    <div className="px-4 py-5 sm:px-6 bg-gray-50 border-b border-gray-200">
                                    ...
                                    <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
                                        <dl className="sm:divide-y sm:divide-gray-200">
                                            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                ...
                                                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{previousSchoolName || '—'}</dd>
                                            </div>
                                            {(previousSchoolLastClass || previousSchoolYear) && (
                                                <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    <dt className="text-sm font-medium text-gray-500">Last Class Attended</dt>
                                                    <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{previousSchoolLastClass || '—'}</dd>
                                                </div>
                                            )}
                                            {previousSchoolYear && (
                                                <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    <dt className="text-sm font-medium text-gray-500">Year Completed</dt>
                                                    <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{previousSchoolYear}</dd>
                                                </div>
                                            )}
                                    ...
                                    <div className="border-t border-gray-200 px-4 py-5 sm:p-6">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {docs.map((doc) => (
                                                <a
                                                    key={doc.id}
                                                    href={route('principal.applications.documents.show', [application.id, doc.id])}
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="flex items-center justify-between rounded-lg border border-gray-200 px-4 py-3 text-sm font-medium text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50 transition"
                                                >
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white shadow rounded-lg p-6">
                                ...
                                <div className="mt-6 pt-6 border-t border-gray-200">
                                    ...
                                    <SecondaryButton onClick={() => window.open(route('principal.applications.print', application.id), '_blank')} className="w-full justify-center mt-3">
                                        Download PDF
                                    </SecondaryButton>
                                </div>
                            </div>
