...
export default function SchoolLogin({ school, role, roleLabel }) {
    ...
    const config = school?.slug ? getSchoolConfig(school.slug) : null;
    const displayName = config?.brandName || school?.name || 'One-Skul';
    const { data, setData, post, processing, errors, reset } = useForm({
        email: '',
        password: '',
        remember: false,
        school_id: school?.id ?? null, // Optional school context
    ...
    return (
        <GuestLayout>
            <Head title={`${roleLabel} Login - ${displayName}`} />
            ...
            <form onSubmit={submit} className="space-y-6">
                <div>
                    ...
                    <TextInput
                        ...
                        placeholder={`${role}@${(school?.name || 'oneskul').toLowerCase().replace(/\s/g, '')}.edu`}
                    />
