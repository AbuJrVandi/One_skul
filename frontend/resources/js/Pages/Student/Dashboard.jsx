...
export default function Dashboard({ auth, student, school, class: schoolClass, notices, decisionDocuments = [] }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                        {/* Status Cards */}
                        <div className="lg:col-span-3 space-y-8 self-start">
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-sm font-black text-gray-700 uppercase tracking-widest">Admission Documents</h3>
                                    {decisionDocuments.length > 0 && (
                                        <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">Available</span>
                                    )}
                                </div>
                                {decisionDocuments.length === 0 ? (
                                    <div className="py-6 text-center text-sm text-gray-500 italic">
                                        No decision document available yet.
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {decisionDocuments.map((doc) => (
                                            <a
                                                key={doc.id}
                                                href={route('student.decision-documents.show', doc.id)}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="flex items-center justify-between rounded-2xl border border-gray-100 px-4 py-3 text-sm font-semibold text-indigo-700 hover:bg-indigo-50"
                                            >
                                                <div>
                                                    <div>{doc.decision === 'accepted' ? 'Admission Letter' : 'Decision Letter'}</div>
                                                    <div className="text-xs text-gray-400">{doc.generated_at || '—'}</div>
                                                </div>
