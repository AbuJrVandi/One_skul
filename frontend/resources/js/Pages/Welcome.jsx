                        ...
                        <main className="mt-16">
                            <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-gray-100 text-center animate-in zoom-in duration-700">
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                    {!auth.user && (
                                        <Link href={route('admin.login')} className="bg-indigo-600 text-white px-10 py-5 rounded-2xl font-black text-lg shadow-xl shadow-indigo-100 hover:scale-105 transition-all">
                                            Administrator Login
                                        </Link>
                                    )}
                                    <Link href={route('apply.discovery')} className="bg-white text-gray-900 border-2 border-gray-100 px-10 py-5 rounded-2xl font-black text-lg hover:bg-gray-50 transition-all">
                                        Apply to a School
                                    </Link>
                                </div>
                            </div>
